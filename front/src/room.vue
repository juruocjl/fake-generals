...
var socket = io("http://"+location.hostname+":3000/room"+window.location.pathname.split('/').pop(),{auth:{
	userid:userid,
	username:user,
	pswd:pswd,
	rating:rating,
...
watch(weather,(newvalue)=>{
	...
	socket.emit('weather change',{'type':newvalue});
})
function showrank(){
	ranking.value=true;
...
function setnow(a,b){
	if(nowx.value!=-1&&nowy.value!=-1&&Math.abs(nowx.value-a)+Math.abs(nowy.value-b)==1){
		...
		socket.emit('add queue',{'data':[nowx.value,nowy.value,op.value*4+d]})
		nowx.value=a,
		nowy.value=b,
		op.value=0;
		...
		socket.emit('add queue',{'data':[nowx.value,nowy.value,a,b,op.value]})
...
socket.on('new turn',(data)=>{
	if(notinit)socket.emit('get map',{'full':true});
	else socket.emit('get map',{'full':false});
...
<template>
	...
	<div class="index" v-if="!start">
		<div class="contain">
			...
			<el-button-group style="margin:10px;">
			<el-button style="margin:10px;" type="success" plain @click="socket.emit('startgame');">Start!</el-button>
			</el-button-group><br>
